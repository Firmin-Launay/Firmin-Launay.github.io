...
function set_light_mode() {
    ...
    root.style.setProperty("--title-color", "#3A3A3A");
    root.style.setProperty("--subtitle-color", "#171717");
    root.style.setProperty("--background-color", "#FFFFFF");
    root.style.setProperty("--image-invert", "invert(0%)");
}

function set_dark_mode() {
    let root = document.documentElement;
    root.style.setProperty("--title-color", "#FFFFFF");
    root.style.setProperty("--subtitle-color", "#C5C5C5");
    root.style.setProperty("--background-color", "#22202D");
    root.style.setProperty("--image-invert", "invert(100%)");
}

function which_mode(autochange=true) {
...
function is_on_mobile(autochange=true) {
    ...
    if (check && autochange) {
        ...
        window.fitText(document.getElementById("title"), 1.5);
        var elements_with_id = document.querySelectorAll('*[id]');
        for (const element of elements_with_id) {
            if (element["id"].startsWith("prj_url_")) {
                window.fitText(element, 1.5);
            }
        }
    }
    return check;
